...
template <class T>
[[clang::matcher("assign")]]
auto FakeClass::decl_m() {
	[[clang::rewrite_setup]]
		int align, array_size;

	[[clang::matcher_block]] {
		this->a = (T*)aligned_alloc(align, sizeof(T)*array_size);
	}
}

template <class T>
[[clang::replace("assign")]]
auto FakeClass::decl_r() {
  ...
  [[clang::rewrite_setup]]
    T* a;
  [[clang::rewrite_setup]]
    T* device_a = &clang_rewrite::code_literal("device_" + clang_rewrite::to_str(a));
  [[clang::rewrite_setup]]
    T* hostmirror_a = &clang_rewrite::code_literal("hostmirror_" + clang_rewrite::to_str(a));

  [[clang::matcher_block]] {
    device_a = new Kokkos::View<T*>("label change me", array_size);
